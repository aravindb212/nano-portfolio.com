...
if(motionToggle) motionToggle.addEventListener("click", () => {
  ...
  if(typeof syncMotionAnimation === "function") syncMotionAnimation();
});
applyMotionPreference();
...
let particles = [], width, height, animationFrame = null, pixelRatio=1;
function resize(){
  ...
  particles=Array.from({length:Math.min(75,Math.floor(width/18))},()=>({x:Math.random()*width,y:Math.random()*height,vx:(Math.random()-.5)*.30,vy:(Math.random()-.5)*.30,r:Math.random()*1.45+.5}));
}
function draw(){
  ...
  particles.forEach(p=>{
    ...
    ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=`rgba(${color},.58)`;ctx.fill();
    particles.forEach(q=>{const d=Math.hypot(p.x-q.x,p.y-q.y);if(d<125){ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.strokeStyle=`rgba(${color},${.14*(1-d/125)})`;ctx.lineWidth=.65;ctx.stroke();}});
  });
  animationFrame=requestAnimationFrame(draw);
}
function syncMotionAnimation(){
  if(document.body.classList.contains("reduce-motion")){if(animationFrame!==null)cancelAnimationFrame(animationFrame);animationFrame=null;return;}
  if(animationFrame===null)draw();
}
resize(); addEventListener("resize",resize); syncMotionAnimation();
...
(function(){
  ...
  if(title&&window.gsap){
    ...
    const renderScene=(i)=>{
      ...
      gsap.fromTo([title,text],{y:reducedMotion?0:10,opacity:reducedMotion?1:.35},{y:0,opacity:1,duration:reducedMotion?.01:.32,ease:'power2.out',stagger:reducedMotion?0:.04});
    };
    renderScene(0);
    if (vision) {
  let touchStartY = 0;
  ...
  document
    ...
    gsap.to('.story-core',{rotation:reducedMotion?0:360,scale:reducedMotion?1:1.08,ease:'none',scrollTrigger:{trigger:'#vision',start:'top top',end:'bottom bottom',scrub:1}});
    gsap.to('.story-ring',{rotation:reducedMotion?0:180,ease:'none',scrollTrigger:{trigger:'#vision',start:'top top',end:'bottom bottom',scrub:1}});
  }
...
(() => {
  ...
  function parallax(){
    if(document.body.classList.contains('reduce-motion')){ canvas.style.transform='none'; requestAnimationFrame(parallax); return; }
